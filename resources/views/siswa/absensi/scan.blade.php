...
<script type="text/javascript">
    ...
    function lihatPosisi(posisi){
        console.log(posisi)
        localStorage.setItem('latitude', posisi.coords.latitude)
        localStorage.setItem('longitude', posisi.coords.longitude)
    }
    ...
    function onScanSuccess(decodedText, decodedResult) {
            $.ajax({
            url:"{{ route('siswa-scanning-absensi') }}",
            type:"POST",
            data:{
                '_token':"{{ @csrf_token() }}",
                'siswa':decodedText,
            ...
            success:function(e){
                ...
                if(statusCode === 200){
                    ...
                    document.getElementById('nama').innerHTML = data.message
                }else{
                    $('#gagalAlert').addClass('d-none')
                    $('#berhasilAlert').addClass('d-none')
                    $('#errorAlert').removeClass('d-none')
